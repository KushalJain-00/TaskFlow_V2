// storage.js — all requests use the logged-in user's JWT so RLS filters by userId
...
const Storage = (() => {
  ...
  async function _get(table) {
    ...
    if (table === 'settings') {
      _cache[table] = rows[0]?.data || { managerName: Auth.getUserName() };
    } else {
  ...
  async function _upsert(table, id, data) {
    ...
    await fetch(_url(table), {
      method: 'POST',
      headers: _headers(),
      body: JSON.stringify({ id, data: { ...data, userId } }),
    });
  }
  ...
  async function saveSettings(settings) {
    ...
    await _upsert('settings', 'singleton_' + Auth.getUserId(), updated);
  }
